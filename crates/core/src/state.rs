...
/// Prepared starting points for a new agent. Each one is a real combination of
/// the patterns, skills, servers and tools the library already describes: the
/// wizard is a shortcut through the editor, never a second source of truth.
fn purposes() -> Vec<AgentPurpose> {
    vec![
        AgentPurpose {
            id: "review".into(),
            name: "Review changes".into(),
            summary: "Reads a diff, reports findings with file and line, and never writes.".into(),
            role: "Review".into(),
            icon: "shield".into(),
            accent: "blue".into(),
            suggested_name: "reviewer".into(),
            instructions: "Read the change and report what is wrong with it. Name the file and the line. Never write to the workspace, and never run a command that changes it.".into(),
            patterns: vec!["read-only".into(), "report-findings".into()],
            skills: vec!["diff-review".into(), "repo-map".into()],
            mcp: vec!["git".into()],
            tools: vec!["snapshot".into(), "inspect".into()],
            model_note: "Reads a lot and writes little: context size matters more than speed.".into(),
        },
        AgentPurpose {
            id: "implement".into(),
            name: "Implement a change".into(),
            summary: "Proposes a diff for one module at a time and waits for approval.".into(),
            role: "Implementation".into(),
            icon: "code".into(),
            accent: "green".into(),
            suggested_name: "builder".into(),
            instructions: "Read the specification before proposing a change. Touch one module per run. Open a diff for review; never write outside the workspace root.".into(),
            patterns: vec!["review-before-write".into(), "one-change-per-run".into()],
            skills: vec!["repo-map".into(), "diff-review".into()],
            mcp: vec!["filesystem".into(), "git".into()],
            tools: vec!["snapshot".into(), "inspect".into(), "agents_in".into()],
            model_note: "Writes code and holds a plan: the strongest model earns its price here.".into(),
        },
        AgentPurpose {
            id: "verify".into(),
            name: "Run the tests".into(),
            summary: "Runs the suite, waits for approval on every command, and reports what failed.".into(),
            role: "Verification".into(),
            icon: "check".into(),
            accent: "amber".into(),
            suggested_name: "tests".into(),
            instructions: "Run the test suite and report failures with the command that produced them. Ask before every shell command. Change nothing.".into(),
            patterns: vec!["test-before-propose".into(), "approval-for-shell".into()],
            skills: vec!["cargo-tests".into()],
            mcp: vec!["filesystem".into()],
            tools: vec!["snapshot".into(), "open_terminal".into()],
            model_note: "Mostly waiting on a process: a local model is enough and costs only power.".into(),
        },
        AgentPurpose {
            id: "design".into(),
            name: "Audit the interface".into(),
            summary: "Checks contrast, focus order and hit targets against the token file.".into(),
            role: "Review".into(),
            icon: "sun".into(),
            accent: "violet".into(),
            suggested_name: "auditor".into(),
            instructions: "Check changed screens for contrast, focus order and hit target size. Read the token file and refuse a colour literal. Report findings; never write.".into(),
            patterns: vec!["read-only".into(), "report-findings".into()],
            skills: vec!["design-tokens".into(), "accessibility-audit".into()],
            mcp: vec!["filesystem".into()],
            tools: vec!["snapshot".into(), "inspect".into()],
            model_note: "Judgement on small inputs: a mid-sized model is enough.".into(),
        },
        AgentPurpose {
            id: "write".into(),
            name: "Write the release notes".into(),
            summary: "Turns commits into prose a person can read, and stops at the diff.".into(),
            role: "Writing".into(),
            icon: "logs".into(),
            accent: "amber".into(),
            suggested_name: "release-notes".into(),
            instructions: "Read the commits since the last tag and write what changed, in plain sentences, for someone who did not write the code. Propose the file as a diff.".into(),
            patterns: vec!["review-before-write".into()],
            skills: vec!["changelog-writer".into()],
            mcp: vec!["git".into()],
            tools: vec!["snapshot".into(), "inspect".into()],
            model_note: "Prose quality is the whole job: the better model reads better.".into(),
        },
        AgentPurpose {
            id: "explore".into(),
            name: "Answer questions about the code".into(),
            summary: "Maps the repository and answers without touching anything.".into(),
            role: "Orientation".into(),
            icon: "search".into(),
            accent: "neutral".into(),
            suggested_name: "guide".into(),
            instructions: "Answer questions about this workspace from what you can read. Quote the file and line you read it from. Write nothing.".into(),
            patterns: vec!["read-only".into()],
            skills: vec!["repo-map".into()],
            mcp: vec!["filesystem".into(), "git".into()],
            tools: vec!["snapshot".into(), "inspect".into(), "agents_in".into()],
...
fn sandboxes() -> Vec<Sandbox> {
    vec![
        ...
        Sandbox {
            ...
            account: Some("agent-review".into()),
            state: "idle".into(),
            machine: "MacBook Pro".into(),
