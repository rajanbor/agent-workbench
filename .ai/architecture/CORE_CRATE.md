...
`crates/core` is a pure Rust crate: it reads no network, spawns no process
...
| --- | --- |
| `core::domain` | Object model: machine, providers, models, agents, sandboxes, sessions, terminals, workflow, capabilities, usage, version control, inspector policy. |
| `core::state` | Deterministic prototype snapshot, labeled as prototype in the UI. |
| `core::inspector` | Scoped, redacting summariser behind `InspectorPolicy`. |
...
`web/src/data/prototype-snapshot.json`. The browser preview reads that file,
so it shows exactly what the engine would return. Regenerate it in the same
change whenever `engine::state` changes.

## Tests

`pnpm test` (`cargo test --workspace`) covers: providers stay disconnected without
